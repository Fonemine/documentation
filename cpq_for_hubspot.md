...
Each **Product Group or Line Item Table** must be configured to only allow adding **Product/Service** items from specific **Product/Service Categories** and can also be constrained to have a **Minimum** and **Maximum** number of items. This means that all  **Product/Service** items that are created from specific **Product/Service Categories** must be added to the **Line Items** table. See the screenshot below on how to do this.
