...
* FILTER(array, var, expr): Returns a subset of the given array filtered
  by the given expression, where 'var' is set to the current element
  of the array.
...
* MAP(array, var, expr): Returns a new array whose elements are generated by applying the given expression on each array element,
  where 'var' is set to the current element
 
* REDUCE(array, prev, curr, expr [, initial]): Reduces the given array
  into a single value using the given expression.  Parameter 'curr'
  is the current array value. Parameter 'prev' is the computed value
  for all previous array elements.  Parameter 'initial' is the
  initial value.
